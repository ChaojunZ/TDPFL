...
save_root_path = r'D:\2024博士工作\FL\code\FLcode\NCODE1\result'
...
rank = [0, 6, 8]
modes = [0, 1, 2]
factors1 = tl.tensor(rng.random_sample((2, rank[1])))
...
for k in range(k_fold):
    ...
    for epoch in range(n_epoch):
        ...
        Model11.train(), Model22.train()
        for site1 in range(1, 3):
            ...
            for _, x in enumerate(dataloader1):
                ...
                if epoch == 0 and site1 == 1:
                    ...
                    factors11 = [factors01] + [factors1] + [factors2]
                else:
                    factors01 = np.identity(data1.size(0))
            ...
            for _, x in enumerate(dataloader2):
                ...
                if epoch == 0 and site1 == 1:
                    ...
                    factors22 = [factors02] + [factors1] + [factors2]
                else:
                    factors02 = np.identity(data2.size(0))
        ...
        parameter_init1 = W_A(para_list, [184, 66])
        parameter_init2 = W_A(para_list, [184, 66])
...
print('NYU Acc: {:3.6f} UM ACC: {:3.6f} UCLA Acc: {:3.6f}'.format(
                    sum(test_acc11)/5, sum(test_acc22)/5
                ))
print('end')
